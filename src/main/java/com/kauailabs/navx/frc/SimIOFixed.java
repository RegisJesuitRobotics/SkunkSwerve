...
class SimIOFixed implements IIOProvider {
    ...
    public SimIOFixed(byte update_rate_hz, IIOCompleteNotification notify_sink, SimDevice sim_device) {
        ...
        if (sim_device != null) {
            ...
            simConnected = sim_device.createBoolean("Connected", Direction.kInput, true);

            // Doubles

            simRate = sim_device.createDouble("Rate", Direction.kInput, 0.0f);
            simYaw = sim_device.createDouble("Yaw", Direction.kInput, 0.0f);
            simPitch = sim_device.createDouble("Pitch", Direction.kInput, 0.0f);
            simRoll = sim_device.createDouble("Roll", Direction.kInput, 0.0f);

            simCompassHeading = sim_device.createDouble("CompassHeading", Direction.kInput, 0.0f);
            simFusedHeading = sim_device.createDouble("FusedHeading", Direction.kInput, 0.0f);

            simLinearWorldAccelX = sim_device.createDouble("LinearWorldAccelX", Direction.kInput, 0.0f);
            simLinearWorldAccelY = sim_device.createDouble("LinearWorldAccelY", Direction.kInput, 0.0f);
            simLinearWorldAccelZ = sim_device.createDouble("LinearWorldAccelZ", Direction.kInput, 0.0f);
        ...
        board_state.capability_flags =
                ...
                AHRSProtocol.NAVX_CAPABILITY_FLAG_OMNIMOUNT | AHRSProtocol.NAVX_CAPABILITY_FLAG_AHRSPOS_TS |
        ...
        // The following values are updated by Sim Variables later; set to reasonable
        // defaults now
