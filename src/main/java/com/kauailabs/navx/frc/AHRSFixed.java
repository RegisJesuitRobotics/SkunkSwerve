...
public class AHRSFixed implements Sendable, Gyro {
     ...
     */
    public AHRSFixed(SPI.Port spi_port_id, byte update_rate_hz) {
        commonInit(update_rate_hz);
        if (m_simDevice != null) {
            io = new SimIOFixed(update_rate_hz, io_complete_sink, m_simDevice);
        } else {
            if (RuntimeDetector.isArm32() && spi_port_id == SPI.Port.kMXP) {
                io = new RegisterIOMau(update_rate_hz, io_complete_sink, board_capabilities);
            } else {
                io = new RegisterIO(
                        new RegisterIO_SPI(new SPI(spi_port_id)), update_rate_hz, io_complete_sink, board_capabilities
                );
            }
        }
    ...
    public AHRSFixed(SPI.Port spi_port_id, int spi_bitrate, byte update_rate_hz) {
        ...
        if (m_simDevice != null) {
            ...
            if (RuntimeDetector.isArm32() && spi_port_id == SPI.Port.kMXP) {
                ...
                io = new RegisterIO(
                        new RegisterIO_SPI(new SPI(spi_port_id), spi_bitrate), update_rate_hz, io_complete_sink,
                        board_capabilities
                );
            }
        }
     ...
     * @param update_rate_hz Custom Update Rate (Hz)
     */
    public AHRSFixed(I2C.Port i2c_port_id, byte update_rate_hz) {
        Tracer.Trace("Instantiating navX-Sensor on I2C Port %s.\n", i2c_port_id.toString());
        commonInit(update_rate_hz);
        if (m_simDevice != null) {
            io = new SimIO(update_rate_hz, io_complete_sink, m_simDevice);
        } else {
            if (RuntimeDetector.isArm32() && i2c_port_id == I2C.Port.kMXP) {
                io = new RegisterIOMau(update_rate_hz, io_complete_sink, board_capabilities);
            } else {
                io = new RegisterIO(
                        new RegisterIO_I2C(new I2C(i2c_port_id, 0x32)), update_rate_hz, io_complete_sink,
                        board_capabilities
                );
            }
        }
     ...
     * @param update_rate_hz Custom Update Rate (Hz)
     */
    public AHRSFixed(SerialPort.Port serial_port_id, SerialDataType data_type, byte update_rate_hz) {
        Tracer.Trace("Instantiating navX-Sensor on Serial Port %s.\n", serial_port_id.toString());
        commonInit(update_rate_hz);
        if (m_simDevice != null) {
            io = new SimIOFixed(update_rate_hz, io_complete_sink, m_simDevice);
        } else {
            if (RuntimeDetector.isArm32() && serial_port_id == SerialPort.Port.kMXP) {
     ...
     * @param spi_port_id SPI port to use.
     */
    public AHRSFixed(SPI.Port spi_port_id) {
     ...
     * @param i2c_port_id I2C port to use
     */
    public AHRSFixed(I2C.Port i2c_port_id) {
     ...
     * @param serial_port_id SerialPort to use
     */
    public AHRSFixed(SerialPort.Port serial_port_id) {
     ...
     *
     * NOTE: This method has no effect if the sensor is currently calibrating, since
     * resetting the yaw will interfere with the calibration process.
     */
    public void zeroYaw() {
     ...
     *                         parameter to the callback object.
    ...
    /**
     * Enables or disables board-level yaw zero (reset) requests. Board-level yaw
     * resets are processed by the sensor board and the resulting yaw angle may not
     * be available to the client software until at least 2 update cycles have
     * occurred. Board-level yaw resets however do maintain synchronization between
     * the yaw angle and the sensor-generated Quaternion and Fused Heading values.
     *
    ...
    /**
     * Returns the sensor full scale range (in degrees per second) of the X, Y and
     * X-axis gyroscopes.
     *
    ...
    class IOCompleteNotification implements IIOCompleteNotification {
        ...
        public void setYawPitchRoll(YPRUpdate ypr_update, long sensor_timestamp) {
            AHRSFixed.this.yaw = ypr_update.yaw;
            AHRSFixed.this.pitch = ypr_update.pitch;
            AHRSFixed.this.roll = ypr_update.roll;
            AHRSFixed.this.compass_heading = ypr_update.compass_heading;
            AHRSFixed.this.last_sensor_timestamp = sensor_timestamp;
        }
        ...
        public void setAHRSPosData(AHRSPosUpdate ahrs_update, long sensor_timestamp) {
            ...
            AHRSFixed.this.yaw = ahrs_update.yaw;
            AHRSFixed.this.pitch = ahrs_update.pitch;
            AHRSFixed.this.roll = ahrs_update.roll;
            AHRSFixed.this.compass_heading = ahrs_update.compass_heading;
            yaw_offset_tracker.updateHistory(ahrs_update.yaw);

            /* Update AHRS class variables */

            // 9-axis data
            AHRSFixed.this.fused_heading = ahrs_update.fused_heading;

            // Gravity-corrected linear acceleration (world-frame)
            AHRSFixed.this.world_linear_accel_x = ahrs_update.linear_accel_x;
            AHRSFixed.this.world_linear_accel_y = ahrs_update.linear_accel_y;
            AHRSFixed.this.world_linear_accel_z = ahrs_update.linear_accel_z;

            // Gyro/Accelerometer Die Temperature
            AHRSFixed.this.mpu_temp_c = ahrs_update.mpu_temp;

            // Barometric Pressure/Altitude
            AHRSFixed.this.altitude = ahrs_update.altitude;
            AHRSFixed.this.baro_pressure = ahrs_update.barometric_pressure;

            // Status/Motion Detection
            AHRSFixed.this.is_moving = (((ahrs_update.sensor_status & AHRSProtocol.NAVX_SENSOR_STATUS_MOVING) != 0)
                    ? true
                    : false);
            AHRSFixed.this.is_rotating = (((ahrs_update.sensor_status
                    & AHRSProtocol.NAVX_SENSOR_STATUS_YAW_STABLE) != 0) ? false : true);
            AHRSFixed.this.altitude_valid = (((ahrs_update.sensor_status
                    & AHRSProtocol.NAVX_SENSOR_STATUS_ALTITUDE_VALID) != 0) ? true : false);
            AHRSFixed.this.is_magnetometer_calibrated = (((ahrs_update.cal_status
                    & AHRSProtocol.NAVX_CAL_STATUS_MAG_CAL_COMPLETE) != 0) ? true : false);
            AHRSFixed.this.magnetic_disturbance = (((ahrs_update.sensor_status
                    & AHRSProtocol.NAVX_SENSOR_STATUS_MAG_DISTURBANCE) != 0) ? true : false);

            AHRSFixed.this.quaternionW = ahrs_update.quat_w;
            AHRSFixed.this.quaternionX = ahrs_update.quat_x;
            AHRSFixed.this.quaternionY = ahrs_update.quat_y;
            AHRSFixed.this.quaternionZ = ahrs_update.quat_z;
            ...
            updateBoardStatus(
                    ahrs_update.op_status, ahrs_update.sensor_status, ahrs_update.cal_status,
        ...
        public void setRawData(AHRSProtocol.GyroUpdate raw_data_update, long sensor_timestamp) {
            AHRSFixed.this.raw_gyro_x = raw_data_update.gyro_x;
            AHRSFixed.this.raw_gyro_y = raw_data_update.gyro_y;
            AHRSFixed.this.raw_gyro_z = raw_data_update.gyro_z;
            AHRSFixed.this.raw_accel_x = raw_data_update.accel_x;
            AHRSFixed.this.raw_accel_y = raw_data_update.accel_y;
            AHRSFixed.this.raw_accel_z = raw_data_update.accel_z;
            AHRSFixed.this.cal_mag_x = raw_data_update.mag_x;
            AHRSFixed.this.cal_mag_y = raw_data_update.mag_y;
            AHRSFixed.this.cal_mag_z = raw_data_update.mag_z;
            AHRSFixed.this.mpu_temp_c = raw_data_update.temp_c;
        ...
        public void setAHRSData(AHRSProtocol.AHRSUpdate ahrs_update, long sensor_timestamp) {
            ...
            // Magnetometer Data
            AHRSFixed.this.cal_mag_x = ahrs_update.cal_mag_x;
            AHRSFixed.this.cal_mag_y = ahrs_update.cal_mag_y;
            AHRSFixed.this.cal_mag_z = ahrs_update.cal_mag_z;
            ...
            updateDisplacement(
                    AHRSFixed.this.world_linear_accel_x, AHRSFixed.this.world_linear_accel_y, update_rate_hz,
                    AHRSFixed.this.is_moving
            );
        ...
        public void setBoardState(BoardState board_state, boolean update_board_status) {
            ...
            if (update_board_status) {
                updateBoardStatus(
                        board_state.op_status, board_state.sensor_status, board_state.cal_status,
        ...
        void updateBoardStatus(byte op_status, short sensor_status, byte cal_status, byte selftest_status) {
            ...
            if (AHRSFixed.this.op_status == AHRSProtocol.NAVX_OP_STATUS_NORMAL) {
            ...
            if (((AHRSFixed.this.cal_status
                ...
                if (poweron_init_completed || AHRSFixed.this.disconnect_startupcalibration_recovery_pending) {
                    AHRSFixed.this.disconnect_startupcalibration_recovery_pending = false;
                    AHRSFixed.this.yaw_angle_tracker.init();
                    Tracer.Trace("navX-Sensor Yaw angle auto-reset to 0.0 due to startup calibration.\n");
                }
            }

            AHRSFixed.this.op_status = op_status;
            AHRSFixed.this.sensor_status = sensor_status;
            AHRSFixed.this.cal_status = cal_status;
            AHRSFixed.this.selftest_status = selftest_status;
        }

        @Override
        public void yawResetComplete() {
            AHRSFixed.this.yaw_angle_tracker.reset();
            if (AHRSFixed.this.enable_boardlevel_yawreset) {
        ...
        public void disconnectDetected() {
            ...
            AHRSFixed.this.op_status = AHRSProtocol.NAVX_OP_STATUS_ERROR;
            AHRSFixed.this.sensor_status = 0; /* Clear all sensor status flags */
            /*
             * Flag the need to watch for a startup calibration completion event upon later
             * reconnect.
             */
            AHRSFixed.this.disconnect_startupcalibration_recovery_pending = true;
